...
model PVCExample "experiment to test response of ModularConductionX to PVCs"
  ...
  parameter Modelica.SIunits.Period T_normal = if with_sinus then 0.8 else con.pace_av.T "normal cycle duration without PVC";
  Modelica.SIunits.Duration t_since_sig = time - pre(sig_last) "time since last signal from SA/AV node";
  Boolean pvc_a = pre(count_sig) == 5 and t_since_sig > con.delay_sa_v.T_avc0 / 2
    "timer for PVC a): while 6th beat is delayed";
  Boolean pvc_b = pre(count_sig) == 12 and t_since_sig > con.refrac_av.T_refrac / 2
